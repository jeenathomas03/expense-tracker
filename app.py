...
# Secret key is used for session encryption (login session security)
...
def get_db_connection():
    ...
    # Allows database rows to be accessed like dictionary
    # Example: expense["amount"] instead of index numbers
...
def create_table():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS expenses (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            username TEXT NOT NULL,  # Links expense to logged-in user
...
def login():
    ...
    # POST runs when login form is submitted
    if request.method == "POST":
        ...
        if user:
            # Store username in session to maintain login state
...
def logout():
    ...
    # Remove user session (logs out user)
...
def home():
    ...
    # Restrict homepage to logged-in users only
    if "user" not in session:
        return redirect("/login")
...
# __________ VIEW CURRENT MONTH EXPENSES __________
...
def view_expenses():
    ...
    # Fetch expenses only for logged-in user and selected month
...
# __________ EDIT EXPENSE __________
...
def edit_expense(expense_id):
    ...
    # Security check → user can edit only their expenses
...
# __________ DELETE EXPENSE __________
...
def delete_expense(expense_id):
    ...
    # Security check → user can delete only their expenses
    expense = conn.execute(
        "SELECT * FROM expenses WHERE id=? AND username=?", 
        (expense_id, session["user"])
    ).fetchone()
...
# __________ PAST MONTH EXPENSES __________
...
def past_month_expenses():
    ...
    # Get selected month from URL query parameter
    selected_month = request.args.get("month")

    today = datetime.today()

    # If user selects specific month
    if selected_month:
    ...
    # Default → show previous month expenses
    else:
    ...
    # Fetch expenses within selected date range
    expenses = conn.execute("""
        SELECT * FROM expenses 
        WHERE username = ? AND date BETWEEN ? AND ?
        ORDER BY date DESC
...
# __________ MONTHLY GRAPH DATA __________
...
# __________ RUN APPLICATION __________
